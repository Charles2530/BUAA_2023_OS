...
int init_stack(u_int child, char** argv, u_int* init_sp) {
    ...
    *init_sp = USTACKTOP - UTEMP - BY2PG + (u_int)pargv_ptr;
    ...
    if ((r = syscall_mem_map(0, (void*)UTEMP, child, (void*)(USTACKTOP - BY2PG),
                             PTE_D)) < 0) {
        goto error;
    }
...
static int spawn_mapper(void* data,
                        u_long va,
                        size_t offset,
                        u_int perm,
                        const void* src,
    ...
    if (src != NULL) {
        int r =
            syscall_mem_map(child_id, (void*)va, 0, (void*)UTEMP, perm | PTE_D);
        if (r) {
...
int spawn(char* prog, char** argv) {
    ...
    if ((fd = open(prog, O_RDONLY)) < 0) {
        ...
        if (prog[0] != '/') {
            ext_path[0] = '/';
            strcpy(ext_path + 1, prog);
        } else {
            strcpy(ext_path, prog);
        }
        int fd_len = strlen(ext_path);
        ext_path[fd_len] = '.';
        ext_path[fd_len + 1] = 'b';
        ext_path[fd_len + 2] = 0;
        if ((fd = open(ext_path, O_RDONLY)) < 0) {
            return fd;
        }
    }

    // Step 2: Read the ELF header (of type 'Elf32_Ehdr') from the file into
    // 'elfbuf' using 'readn()'. If that fails (where 'readn' returns a
    // different size than expected), set 'r' and 'goto err' to close the
    ...
    // Step 3: Create a child using 'syscall_exofork()' and store its envid
    // in 'child'. If the syscall fails, set 'r' and 'goto err'.
    u_int child;
    ...
    //
    // Step 4: Use 'init_stack(child, argv, &sp)' to initialize the stack of
    // the child. 'goto err1' if that fails.
    u_int sp;
    ...
    ELF_FOREACH_PHDR_OFF(ph_off, ehdr) {
        // Read the program header in the file with offset 'ph_off' and
        // length 'ehdr->e_phentsize' into 'elfbuf'. 'goto err1' on failure.
        ...
        if (ph->p_type == PT_LOAD) {
            void* bin;
            // Read and map the ELF data in the file at 'ph->p_offset' into
            // our memory using 'read_map()'. 'goto err1' if that fails.
            /* Exercise 6.4: Your code here. (5/6) */
            if ((r = read_map(fd, ph->p_offset, &bin)) < 0) {
                goto err1;
            }
            //
            // Load the segment 'ph' into the child's memory using
            // 'elf_load_seg()'. Use 'spawn_mapper' as the callback, and
            // '&child' as its data. 'goto err1' if that fails.
    ...
    // Pages with 'PTE_LIBRARY' set are shared between the parent and the
    // child.
...
int spawnl(char* prog, char* args, ...) {
    // Thanks to MIPS calling convention, the layout of arguments on the
    // stack are straightforward.
