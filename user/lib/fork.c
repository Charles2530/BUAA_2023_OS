...
static void __attribute__((noreturn)) cow_entry(struct Trapframe* tf) {
    ...
    if (!(perm & PTE_COW)) {
        user_panic("IN cow_entry:perm doesn't have PTE_COW");
    }
    //
    /* Step 2: Remove 'PTE_COW' from the 'perm', and add 'PTE_D' to it. */
    /* Exercise 4.13: Your code here. (2/6) */
    perm = ((perm) & (~PTE_COW) & 0xfff) | PTE_D;
    //
 ...
 * 'vpn' * 'BY2PG') in our (current env's) address space. 'PTE_COW' should be
 * used to isolate the modifications on unshared memory from a parent and its
 *   children.
 *
 * Post-Condition:
 ...
 * marked 'PTE_COW' and without 'PTE_D', while the other permission bits are
 * kept.
 *
 ...
 *   - You should use 'syscall_mem_map', the user space wrapper around
 * 'msyscall' to invoke 'sys_mem_map' in kernel.
 */
static void duppage(u_int envid, u_int vpn) {
    ...
    if ((perm & PTE_D) && !(perm & PTE_LIBRARY)) {
        perm = perm & (~PTE_D) & 0xfff;
        // debugf("perm:%x\n",perm);
        // debugf("addr:%x\n",addr);
        try(syscall_mem_map(0, (void*)addr, envid, (void*)addr,
                            perm | PTE_COW));
        try(syscall_mem_map(0, (void*)addr, 0, (void*)addr, perm | PTE_COW));
    } else {
        // debugf("addr:%x\n",addr);
        try(syscall_mem_map(0, (void*)addr, envid, (void*)addr, perm));
    }
}
 ...
 *   Use 'syscall_set_tlb_mod_entry', 'syscall_getenvid', 'syscall_exofork', and
 * 'duppage'.
 */
int fork(void) {
    u_int child;
    ...
    // Hint: 'env' should always point to the current env itself, so we should
    // fix it to the correct value.
    child = syscall_exofork();
    // debugf("%x\n",child);
    if (child == 0) {
        env = envs + ENVX(syscall_getenvid());  // is curenv? but curenv in kern
        return 0;
    }
