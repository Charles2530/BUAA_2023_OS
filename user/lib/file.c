...
int open(const char* path, int mode) {
    ...
    // cached, using 'fd2data'. Set 'size' and 'fileid' correctly with the
    // value in 'fd' as a 'Filefd'.
    char* va;
    struct Filefd* ffd;
    u_int size, fileid;
...
//  files are memory-mapped, this amounts to a memcpy() surrounded by a
//  little red tape to handle the file size and seek pointer.
static int file_read(struct Fd* fd, void* buf, u_int n, u_int offset) {
    u_int size;
    struct Filefd* f;
    f = (struct Filefd*)fd;
...
int openat(int dirfd, const char* path, int mode) {
    ...
    struct Filefd* ffd;
    u_int fileid;
    ffd = (struct Filefd*)dir;
    fileid = ffd->f_fileid;
    try(fsipc_openat(fileid, path, mode, fd));
    char* va;
    u_int size;
    va = fd2data(fd);
    ffd = (struct Filefd*)fd;
