...
int pipe(int pfd[2]) {
    ...
    if ((r = syscall_mem_map(0, (void*)va, 0, (void*)fd2data(fd1),
                             PTE_D | PTE_LIBRARY)) < 0) {
        goto err3;
    }
 ...
 *   The return value must be greater than 0, unless the pipe is closed and
...
static int pipe_read(struct Fd* fd, void* vbuf, u_int n, u_int offset) {
    ...
    //  - If at least 1 byte is read, or the pipe is closed, just return the
    //  number
    //    of bytes read so far.
    //  - Otherwise, keep yielding until the buffer isn't empty or the pipe is
    //  closed.
...
static int pipe_write(struct Fd* fd, const void* vbuf, u_int n, u_int offset) {
    ...
    // Use 'fd2data' to get the 'Pipe' referred by 'fd'.
    // Write a loop that transfers one byte in each iteration.
    // If the bytes of the pipe used equals to 'BY2PIPE', the pipe is regarded
    // as full. Check if the pipe is closed by '_pipe_is_closed'. When the pipe
    // buffer is full:
    //  - If the pipe is closed, just return the number of bytes written so far.
    //  - If the pipe isn't closed, keep yielding until the buffer isn't full or
    //  the
    //    pipe is closed.
