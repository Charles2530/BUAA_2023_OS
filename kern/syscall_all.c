...
int sys_set_tlb_mod_entry(u_int envid, u_int func) {
    ...
    try(envid2env(envid, &env, 1));  // unknown
    //
...
int sys_exofork(void) {
    ...
    // memcpy((void*)(&(e->env_tf)),((void*)(KSTACKTOP) - sizeof(struct
...
int sys_ipc_try_send(u_int envid, u_int value, u_int srcva, u_int perm) {
    ...
    if (srcva != 0) {
        ...
        page_insert(e->env_pgdir, e->env_asid, p, dstva, perm);
        //
    }
    return 0;
}
...
int sys_write_dev(u_int va, u_int pa, u_int len) {
    ...
    if (!((pa >= DEV_CONS_ADDRESS &&
           pa + len <= (DEV_CONS_ADDRESS + DEV_CONS_LENGTH)) ||
          (pa >= DEV_DISK_ADDRESS &&
           pa + len <=
               (DEV_DISK_ADDRESS + DEV_DISK_BUFFER + DEV_DISK_BUFFER_LEN)) ||
          (pa >= DEV_RTC_ADDRESS &&
           pa + len <= (DEV_RTC_ADDRESS + DEV_RTC_LENGTH)))) {
        return -E_INVAL;
    }
