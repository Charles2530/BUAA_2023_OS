...
void* alloc(u_int n, u_int align, int clear) {
    ...
    freemem = ROUND(freemem, align);  //>=freemem
...
void page_init(void) {
    ...
    for (now = pa2page(PADDR(freemem)); page2ppn(now) < npage; now++) {
        now->pp_ref = 0;
